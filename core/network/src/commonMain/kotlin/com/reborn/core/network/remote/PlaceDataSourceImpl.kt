...
class PlaceDataSourceImpl(
    ...
    override suspend fun getAdmins(placeId: Long): ApiResponse<PlaceAdminListResponse> = runCatching {
        httpClient.get("/api/place/$placeId/admins")
    }.asApiResponse()

    override suspend fun getWifi(placeId: Long): ApiResponse<PlaceWifiResponse> = runCatching {
        httpClient.get("/api/place/$placeId/wifi")
    }.asApiResponse()

    override suspend fun updateWifi(placeId: Long, request: UpdatePlaceWifiRequest): ApiResponse<PlaceWifiResponse> =
        runCatching {
            httpClient.put("/api/place/$placeId/wifi") {
                setBody(request)
            }
        }.asApiResponse()
