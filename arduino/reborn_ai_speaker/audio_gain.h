...
// 기존 10.0은 "매미소리" 왜곡의 원인을 게인/클리핑으로 오판했을 때의 값 - 실제 원인은 게인이
// 아니라 streamPlayback()의 TLS 스트리밍 읽기가 client.read()의 부분 수신을 그대로 완성된
// 청크로 처리해 16비트 샘플 경계가 밀리던 버그였다(volume_test.ino로 격리 후 확인, #276).
// 그 버그를 고친 뒤 실기기로 비교 테스트해서 찾은 값 = 5.0(2026-08-01).
constexpr float SPEAKER_GAIN = 5.0f;
