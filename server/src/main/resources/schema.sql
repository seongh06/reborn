...
-- ------------------------------------------------
-- 13. device_schedule_rule (2026-08-03, 시간 기반 자동제어 #325)
-- auto_control_rule(#190, 센서 조건 기반)과 별개 축 - 기기당 여러 건 등록 가능해서 device_id에
-- UNIQUE를 걸지 않는다(예: "07시 켜짐" + "22시 꺼짐" 두 규칙을 한 기기에 동시에 둘 수 있어야 함).
-- SmartThings 기기만 대상(서버가 자격증명을 직접 들고 있어 동기 호출 가능) - ARDUINO(IR)는 대기열
-- 적재만 가능해 실제 실행 시점이 폴링 주기에 달려있어 스코프 밖.
-- ------------------------------------------------
